...
vxin[0:135] = 0.0 # initial vy velocities
vyin[0:135] = 0.0 
...
timestep = .125                                                         # Timestep to update positions
TotalTimesteps  = 1500
...
def Update_Velocities(r1,r2,h):                                             
    ...
    vx1 = r1[2] = r1[2] - a_x # Use negative because we define the positive direction as that from particle 2
    vy1 = r1[3] = r1[3] - a_y 
...
for it in range(TotalTimesteps):
    ...
    for i in range(Nparticles):
        ...
        if (y < 0 ) or (y > 50 ):
            ...
            rvec[i][2] =  vx*vmax/vmag
            rvec[i][3] = - vy*vmax/vmag
        if (x < 0 ) or (x > 50 ):
            rvec[i][0] +=  - vx*2*hstep
            rvec[i][1] +=   vy*2*hstep
    ...
    scatts = ax.scatter(rvec[:,0], rvec[:,1], color = infections, linewidth = 1.5)
    
    # Plotting and Annotating each line 
    linePlot, = ax2.plot(totalTime,InfectedPeople,color = 'r', linewidth = .5)
    ...
    linePlot2, = ax2.plot(totalTime,NonInfected, color = 'b', linewidth = .5)
    ...
    linePlot3, = ax2.plot(totalTime, RecoveredPeople, color = 'k', linewidth = .5)
    ...
    ims.append([scatts,linePlot,annotatedl1,linePlot2,annotatedl2,linePlot3,annotatedl3,])
...
Animation.save('P1c_MD.mp4', writer = FFwriter)
